        ...
        <div class="card-body">
            <form action="{{ route('admin.baiviet.sua', ['id' => $baiviet->id]) }}" method="post">
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('diadiem_id') is-invalid @enderror" id="diadiem_id" name="diadiem_id" required>
                        ...
                        @foreach($diadiem as $value)
                            <option value="{{ $value->id }}" {{ ($baiviet->diadiem_id == $value->id) ? 'selected' : '' }}>{{ $value->tendiadiem }}</option>
                        @endforeach
                    </select>
                    @error('diadiem_id')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('doitac_id') is-invalid @enderror" id="doitac_id" name="doitac_id" required>
                        <option value="">-- Chọn (Có thể không chọn) --</option>
                        @foreach($doitac as $value)
                            <option value="{{ $value->id }}" {{ ($baiviet->doitac_id == $value->id) ? 'selected' : '' }}>{{ $value->tendoitac }}</option>
                        @endforeach
                    </select>
                    @error('doitac_id')
                        <div class="invalid-feedback"><strong>{{ $message }}</strong></div>
